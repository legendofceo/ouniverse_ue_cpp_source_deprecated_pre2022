...
#include "../sys/control_ue.h"
#include "../sys/mode_ue.h"
#include "../sys/hud_ue.h"

#include "../sys/memory_manager.h"
#include "../sys/deep_memory.h"
...
void ACosmos_P::BigBang(TEnumAsByte<EBootCMD> InBootCommand, 
					    TSoftClassPtr<UDataManager> DataManagerClass, 
						...
						UObject* WorldCTX, bool bSkipIntros, FString AutoUserID, FString AutoLoadout)
{
	...
	Log = FLogger("Log");
	Log.Write("Official Boot Started", ELogStatus::Expected);
	...
	Control = AControlUE::Initialize(this);
	Mode = AModeUE::Get(this);
	Hud = AHudUE::Get(this);
	...
	DataManager = UDataManager::Create(DataManagerClass, this);
	UserManager = AUserManager::Create(this);
	MemoryManager = AMemoryManager::Create(this);
	IcoManager = UIcoManager::Create(IcoManagerClass, this);

	DeepMemory = ADeepMemory::Create(this);

	LogManager = ALogManager::Create(this);
...
void ACosmos_P::LoadPreData()
{
	...
	if (!UDataManager::Get()->bAtlasesLoaded)
	{
		UDataManager::Get()->LoadAtlases();
	}
